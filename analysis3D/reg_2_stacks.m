...
numFramesStack1 = 1;
numFramesStack2 = 1;
hStackCorr1 = hStackCorr1(9:520,5:516,2:7,1:numFramesStack1);
hStackCorr2 = hStackCorr2(9:520,5:516,2:7,1:numFramesStack2);
...
%magCorrectFactor = -0.008;
magCorrectFactor = magCorrectFactorV(mIdx);
stack1 = magnify_about_point(stack1,1+linspace(-1,1,size(stack1,3))*magCorrectFactor);
stack2 = magnify_about_point(stack2,1+linspace(-1,1,size(stack2,3))*magCorrectFactor);
...
XPowSpec = F1.*conj(F2);
XC = fftshift3(ifft3(XPowSpec./(abs(XPowSpec + eps(single(0))))));
XCminusBackground = abs(XC)-median(abs(XC),3);
[~,vIdx] = max(XCminusBackground,[],'all','linear');
[yPk,xPk,zPk] = ind2sub(size(XC),vIdx);
shiftVector = [yPk-(size(XC,1)/2+1),xPk-(size(XC,2)/2+1),zPk-(size(XC,3)/2+1)];


%% Show cross-correlation
imagesc(XCminusBackground(yPk-15:yPk+15,xPk-15:xPk+15,zPk));axis equal
title(num2str(magCorrectFactor))
drawnow
